...
# S = np.zeros((200, 200), dtype=np.float)

# def compute_score(i, j, users_json):
#     score = 0.0
#     if (orientation_match(users_json[str(i)], users_json[str(j)]) and language_match(users_json[str(i)], users_json[str(j)])):
#         score = compatibilityScoreCompute(users_json[str(i)], users_json[str(j)])
#     return score

# def compute_scores_for_user_range(i, j_start, j_end, users_json):
#     for k in range(j_start, j_end):
#             score = 0.0
#             if (orientation_match(users_json[str(i)], users_json[str(k)]) and language_match(users_json[str(i)], users_json[str(k)])):
#                 score = compatibilityScoreCompute(users_json[str(i)], users_json[str(k)])
...
#                 S[k][i] = score
#             finally:
#                 # Release the lock after writing to the shared score matrix S
#                 lock.release()
...
def compute(users, max_users, top_k, outfile, outfile_ml, users_json):
    ...
    for i in range(0, max_users):
        # num_jobs = 4
        # chunk_size = (max_users - i) // num_jobs
        # for j in range(num_jobs):
        #     print(i, i + 1 + j * chunk_size, i + 1 + (j + 1) * chunk_size)
        # print(i, i + num_jobs * chunk_size, max_users)
        # Parallel(n_jobs=num_jobs)(delayed(compute_scores_for_user_range)(i, i + 1 + j * chunk_size, i + (j + 1) * chunk_size, users_json) for j in range(num_jobs))
        # Handling any remaining users
        # if (max_users - i) % num_jobs != 0:
        #     compute_scores_for_user_range(i, i + num_jobs * chunk_size, max_users, users_json)
    ...
    for i in range(max_users):
        ...
        for j in indexes:
            cusers.append(j)
            cusers_score.append(S[i][j]) #keys[j]
        compatibility[str(i)] = {"index": cusers, "score": cusers_score}

        # indexes_ml = heapq.nlargest(top_k, range(len(S_ml[i])), S_ml[i].__getitem__)
        # cusers_ml = []
        # for j in indexes_ml:
        #     cusers_ml.append(S_ml[i][j])
...
def calculate_compatibility(max_users):
    t0 = time.time()
    top_k = 50 # set how many
    ...
    print ('Extracting', max_users, 'users...')
    ...
    t4 = time.time()
    print ('Done computing compatibility scores! Total time: ', t4-t0, 'secs')
